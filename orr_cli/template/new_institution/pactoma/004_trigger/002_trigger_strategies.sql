CREATE OR REPLACE FUNCTION "{{schema}}".trigger_insert_estra_rest_ma()
RETURNS TRIGGER AS 
$$
DECLARE
    insert_sql TEXT;
...
begin
	  ...
	  delete_est := 'DELETE FROM "{{schema}}".features_estra_rest_ma 
                   WHERE uuid = $1;' ;
    ...
    insert_sql := 'INSERT INTO "{{schema}}".features_estra_rest_ma (uuid, code_key)
                   SELECT DISTINCT ON (mt.uuid)
                          ...
                          CASE 
                              ...
                              ELSE ''ers00''
                          END AS ers_code
                   FROM "{{schema}}".features_met_comb_ma mt
...
FOR EACH ROW -- 
EXECUTE FUNCTION "{{schema}}".trigger_insert_estra_rest_ma();



-- Criar o gatilho
CREATE TRIGGER insert_estrategies_met_comb_{{schema}}
AFTER INSERT OR UPDATE or delete ON "{{schema}}".features_met_comb_ma 
FOR EACH ROW -- 
EXECUTE FUNCTION "{{schema}}".trigger_insert_estra_rest_ma();
