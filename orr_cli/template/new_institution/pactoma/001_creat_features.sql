/* DONE: remover o multipolygon
   ...
   DONE indexes
   DONE PROT MONIT - escrita de nao identificado
...
CREATE TABLE IF NOT EXISTS "{{schema}}".features(
        ...
        origem        varchar(250) default 'Não identificado',
        idpacto       varchar(100) default 'Não identificado',
        ...
        uso_ant varchar(5) default 'usa00',
        ...
        prot_monit   varchar(50) default 'Não identificado',
        freq_monit   varchar(80) default 'Não identificado',
        ano_prev     ob_date,
        ano_prev_t   ob_date,
        ...
        FOREIGN KEY (categoria) REFERENCES options.categoria_ma(code_key),
        FOREIGN KEY (motivacao) REFERENCES options.motivacao_ma(code_key),
        FOREIGN KEY (proj_ativo) REFERENCES options.proj_ativo_ma(code_key),
        FOREIGN KEY (projeto) REFERENCES options.projeto_ma(code_key),
        FOREIGN KEY (met_princ) REFERENCES options.met_princ_ma(code_key),
        FOREIGN KEY (monitorada) REFERENCES options.monitorada_ma(code_key),
        FOREIGN KEY (uso_ant) REFERENCES options.uso_ant_ma(code_key),
        FOREIGN KEY (tipo_area) REFERENCES options.tipo_area_ma(code_key),
        FOREIGN KEY (privac_area) REFERENCES options.privac_area_ma(code_key),
        FOREIGN KEY (fonte_schema) REFERENCES orr_metadata.institutions(name), 
        CONSTRAINT unique_{{schema}}_geohash_dat_inicio UNIQUE (
            geohash,
            categoria,
            projeto,
            dat_inicio, 
...
CREATE INDEX {{schema}}_features_uso_ant_idx ON {{schema}}.features USING btree (uso_ant);


CREATE TABLE IF NOT EXISTS {{schema}}.features_estra_rest_ma
(
    uuid    uuid not null
        constraint fk_{{schema}}_feature_estra_rest_ma_id
            references {{schema}}.features,
    code_key text not null
        constraint fk_{{schema}}_estra_rest_ma_id
            references options.estra_rest_ma,
    constraint pk_features_estra_rest_ma
        primary key (uuid, code_key)
);

CREATE INDEX {{schema}}_features_estra_rest_ma_uuid_idx ON {{schema}}.features_estra_rest_ma USING btree (uuid);
CREATE INDEX {{schema}}_features_estra_rest_ma_code_key_idx ON {{schema}}.features_estra_rest_ma USING btree (code_key);


CREATE TABLE IF NOT EXISTS {{schema}}.features_met_comb_ma
(
    uuid    uuid not null
    ...
    code_key text not null
        constraint fk_{{schema}}_met_comb_id
            references options.met_comb_ma,
    constraint pk_features_met_comb
        primary key (uuid, code_key)
);

CREATE INDEX {{schema}}_features_met_comb_ma_uuid_idx ON {{schema}}.features_met_comb_ma USING btree (uuid);
CREATE INDEX {{schema}}_features_met_comb_ma_code_key_idx ON {{schema}}.features_met_comb_ma USING btree (code_key);


alter table "{{schema}}".features
    owner to orr_master;

alter table "{{schema}}".features_met_comb_ma
    owner to orr_master;

alter table "{{schema}}".features_estra_rest_ma
    owner to orr_master;
        
GRANT ALL PRIVILEGES ON TABLE "{{schema}}".features TO {{schema}}_user;
GRANT ALL PRIVILEGES ON TABLE "{{schema}}".features_met_comb_ma TO {{schema}}_user;
GRANT ALL PRIVILEGES ON TABLE "{{schema}}".features_estra_rest_ma TO {{schema}}_user;

GRANT USAGE ON SCHEMA "{{schema}}" TO PUBLIC;
GRANT SELECT ON TABLE "{{schema}}".features TO PUBLIC;
GRANT SELECT ON TABLE "{{schema}}".features_estra_rest_ma TO PUBLIC;
GRANT SELECT ON TABLE "{{schema}}".features_met_comb_ma  TO PUBLIC;
